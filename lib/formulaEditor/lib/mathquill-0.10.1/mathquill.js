...
(function () {
     ...
     * TODO: rethink this.
     */
    function bind(cons /*, args... */) {      //bind:绑定
...
// and (-L) === R, and (-R) === L.
    ...
    /*LaTeX命令的注册表以及键入单个字符时创建的命令*/
    /**
     * Registry of LaTeX commands and commands created when typing
     * a single character.
     *
     * (Commands are all subclasses of Node.) //命令都是Node的所有子类。
     */
    var LatexCmds = {}, CharCmds = {};
    /********************************************
     * Cursor and Selection "singleton" classes   //游标和选择“单例”类
     *******************************************/
...
//A fake cursor in the fake textbox that the math is rendered in.
    ...
    /*********************************************************
     * The publicly exposed MathQuill API.                     //公开公开的MathQuill API。
     ********************************************************/
...
// globally exported API object
    ...
    var Parser = P(function (_, super_, Parser) {
        ...
        _.parse = function (stream) {
            //success 调用success(stream, result)函数 !!! 中间变量
            //parseError 调用解析错误的函数
            //''+stream : 把stream转成字符串
...
// Parser MathBlock
     ...
     *  数学树节点基类。
     *  一些特定于数学树的对Node的扩展。
     *  MathBlock和MathCommand都从其继承。
     */
    //MathElement类继承自Node类，并拓展Node类
    //MathBlock和MathCommand都继承自MathElement类。
    /**
  ...
  */
    ...
    // 轻量级命令，无块或子级。 Symbol:符号 继承MathCommand类,重写相关方法
    /**
    ...
    //VanillaSymbol:香草符号  继承自Symbol类,重写 _.init方法
    var VanillaSymbol = P(Symbol, function (_, super_) {
        _.init = function (ch, html) {
            super_.init.call(this, ch, '<span>' + (html || ch) + '</span>');
        };
    });
    //BinaryOperator:二进制运算符 继承自Symbol类,重写 _.init方法
    ...
    // var BinaryOperator1 = P(Symbol, function(_, super_) {
    //   _.init = function(ctrlSeq, html, text) {
    //     super_.init.call(this,
    //         ctrlSeq, '<big class="mq-binary-operator">'+ html+'</big>'+ '<span class="mq-non-leaf mq-summ-stem">&0</span>', text);
    //   };
    // });
    ...
    /*************************************************
     * Abstract classes of text blocks                  //文本块的抽象类
     ************************************************/
     ...
     */
    /*
  * 纯文本块，带有一个或两个TextPiece作为子元素。
  * 表示通常为衬线字体罗马字符的扁平字符串，而不是分层的，嵌套的，树状结构的数学运算符。
  * 包装单个HTMLSpanElement。
  * */
    //  继承Node类，并重写相关方法
...
//继承RootMathBlock类，并重写相关方法
    ...
    /****************************************
     * Input box to type backslash commands    //输入框键入反斜杠命令
     ***************************************/
//继承MathCommand类，重写相关的方法
...
//fonts
    ...
    LatexCmds.dot = P(MathCommand, function (_, super_) {   //继承MathCommand类，重写相关方法
        _.init = function () {
...
//继承MathCommand类，并重写相关方法
    var SupSub = P(MathCommand, function (_, super_) {   //上下标
        _.ctrlSeq = '_{...}^{...}';
        _.createLeftOf = function (cursor) {
...
//下标
    /*LatexCmds.subscript =
LatexCmds._ = P(SupSub, function(_, super_) {   //继承SupSub类，重写相关方法
  _.supsub = 'sub';
  _.htmlTemplate =
...
//上标
    LatexCmds.superscript =
        LatexCmds.supscript =
            LatexCmds['^'] = P(SupSub, function (_, super_) {   //继承SubSub类，重写相关方法
                _.supsub = 'sup';
                _.htmlTemplate =
    ...
    /*later*/
    var SummationNotation_1 = P(SummationNotation, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf" style="vertical-align: 2em;">'
                + '<span class="mq-to"><span>&1</span></span>'
                + '<big>' + html + '</big>'
                + '<span class="mq-from"><span>&0</span></span>'
                + '</span>';
    ...
    /*added by Zhang Yibiao*/
    var SummationNotation1 = P(SummationNotation, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate = '<span class="mq-large-operator mq-non-leaf">'
                + '<span class="mq-to"><span>&1</span></span>'
                + '<big>' + html + '</big>'
                + '<span class="mq-from"><span>&0</span></span>' + '</span>';
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };
    });
    /*added by Zhang Yibiao*/
    //求和符号
    var Summ = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-non-leaf">' +
                '<big>' + html + '</big>' +
                '<span class="mq-non-leaf mq-summ-stem">&0</span>' +
                // '<span class="mq-non-leaf mq-summ-stem">&0</span>' +   !!!important
                '</span>'
    ...
    /*added by Zhang Yibiao*/
    var Summ1 = P(Summ, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-non-leaf">' +
                '<span class="mq-non-leaf mq-summ-stem">&0</span>' +
    ...
    /*added by Zhang Yibiao*/
    LatexCmds.summ = bind(Summ, '\\summ ', '&sum;');
    ...
    var Bracket = P(P(MathCommand, DelimsMixin), function (_, super_) {
        ...
        _.html = function () { // wait until now so that .side may
            this.htmlTemplate = // be set by createLeftOf or parser
                /*'<span class="mq-non-leaf" style="vertical-align: 0.5em">'*/
                '<span class="mq-non-leaf">' /*later*/
...
// 继承自Environment类
    var Matrix =
        Environments.matrix = P(Environment, function (_, super_) {
            ...
            // Create default 4-cell matrix  //创建默认的4单元矩阵
            _.createBlocks = function () {
                this.blocks = [
                    MatrixCell(0, this),
                    MatrixCell(0, this),
...
//添加特定于矩阵的键盘命令
    ...
    LatexCmds.notin =
        LatexCmds.cong =
            LatexCmds.equiv =
                LatexCmds.oplus =
                    LatexCmds.otimes = P(BinaryOperator, function (_, super_) {  //继承自BinaryOperator类
    ...
    LatexCmds.nsub = LatexCmds.notsub =
        LatexCmds.nsubset = LatexCmds.notsubset = bind(BinaryOperator, '\\not\\subset ', '&#8836;');
    LatexCmds.nsup = LatexCmds.notsup =
        LatexCmds.nsupset = LatexCmds.notsupset =
            LatexCmds.nsuperset = LatexCmds.notsuperset = bind(BinaryOperator, '\\not\\supset ', '&#8837;');
    LatexCmds.sube = LatexCmds.subeq = LatexCmds.subsete = LatexCmds.subseteq = bind(BinaryOperator, '\\subseteq ', '&sube;');
    LatexCmds.supe = LatexCmds.supeq =
        LatexCmds.supsete = LatexCmds.supseteq =
            LatexCmds.supersete = LatexCmds.superseteq = bind(BinaryOperator, '\\supseteq ', '&supe;');
    LatexCmds.nsube = LatexCmds.nsubeq =
        LatexCmds.notsube = LatexCmds.notsubeq =
            LatexCmds.nsubsete = LatexCmds.nsubseteq =
                LatexCmds.notsubsete = LatexCmds.notsubseteq = bind(BinaryOperator, '\\not\\subseteq ', '&#8840;');
    LatexCmds.nsupe = LatexCmds.nsupeq =
        LatexCmds.notsupe = LatexCmds.notsupeq =
            LatexCmds.nsupsete = LatexCmds.nsupseteq =
                LatexCmds.notsupsete = LatexCmds.notsupseteq =
                    LatexCmds.nsupersete = LatexCmds.nsuperseteq =
                        LatexCmds.notsupersete = LatexCmds.notsuperseteq = bind(BinaryOperator, '\\not\\supseteq ', '&#8841;');
...
//delimiters
    ...
    // LatexCmds.integral = bind(Summ, '\\int ', '&int;');  //!!! added

    // LatexCmds.intbase = bind(BinaryOperator,'\\intbase ','&int;')  //!!! added
    //*************added*************
    ...
    /*added at 2020-06-23 14:48*/
    LatexCmds.xcup = bind(BinaryOperator, '\\xcup ', '&#8899;');
    LatexCmds.xcap = bind(BinaryOperator, '\\xcap ', '&#8898;');
    /*added at 2020-06-23 14:48*/
...
//semantically should be &sdot;, but &middot; looks better
    ...
    ///////////////////////////////////////////////////////////////
    // ADDED
    // Our work ------  Xu Zhenjie & Wang Jianqiang modified by Zhang Yibiao
    //
    ///////////////////////////////////////////////////////////////
    //11
    var LeftRight = P(MathCommand, function (_, super_) {
        _.init = function (ch, htmll, htmlr) {   //初始化
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<big>' + htmll + '</big>' +
                '<span class="mq-non-leaf ">&0</span>' +
    ...
    LatexCmds.angle = bind(LeftRight, '\\angle ', '&#9001;', '&#9002;');
    LatexCmds.DoubleVerticalLine = bind(LeftRight, '\\DoubleVerticalLine ', '&#8214;', '&#8214;');
    LatexCmds.floor = bind(LeftRight, '\\floor ', '&#8970;', '&#8971;');
    LatexCmds.ceil = bind(LeftRight, '\\ceil ', '&#8968;', '&#8969;');
    LatexCmds.VerticalLineAngle = bind(LeftRight, '\\VerticalLineAngle ', '&#124', '&#9002;');
    LatexCmds.AngleVerticalLine = bind(LeftRight, '\\AngleVerticalLine', '&#9001', '&#124;');
    LatexCmds.LeftSquare = bind(LeftRight, '\\LeftSquare', '&#91;', '&#91;');
    LatexCmds.RightSquare = bind(LeftRight, '\\RightSquare', '&#93;', '&#93;');
    LatexCmds.RightLeftSquare = bind(LeftRight, '\\RightLeftSquare', '&#93;', '&#91;');
    LatexCmds.WhiteSquare = bind(LeftRight, '\\WhiteSquare', '&#10214;', '&#10215;')

    var Left = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {  //初始化
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<big>' + html + '</big>' +
                '<span class="mq-non-leaf ">&0</span>' +
    ...
    var Right = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {  //初始化
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<span class="mq-non-leaf ">&0</span>' +
    ...
    LatexCmds.LeftParenthesis = bind(Left, '\\LeftParenthesis ', '&#40');
    LatexCmds.RightParenthesis = bind(Right, '\\RightParenthesis', '&#41');
    LatexCmds.SingleLeftSquare = bind(Left, '\\SingleLeftSquare', '&#91;');
    LatexCmds.SingleRightSquare = bind(Right, '\\SingleRightSquaree', '&#93;');
    LatexCmds.SingleLeftCurly = bind(Left, '\\SingleLeftCurly', '&#123;');
    LatexCmds.SingleRightCurly = bind(Right, '\\SingleRightCurly', '&#125;');
    LatexCmds.SingleLeftAngle = bind(Left, '\\SingleLeftAngle', '&#9001;');
    LatexCmds.SingleRightAngle = bind(Right, '\\SingleRightAngle', '&#9002;');
    LatexCmds.SingleLeftLine = bind(Left, '\\SingleLeftLine', '&#124;');
    LatexCmds.SingleRightLine = bind(Right, '\\SingleRightLine', '&#124;');
    LatexCmds.LeftDoubleLine = bind(Left, '\\LeftDoubleLine', '&#8214;');
    LatexCmds.RightDoubleLine = bind(Right, '\\RightDoubleLine', '&#8214;');
    LatexCmds.LeftWhiteSquare = bind(Left, '\\LeftWhiteSquare', '&#10214;');/*1128*/
    LatexCmds.RightWhiteSquare = bind(Right, '\\RightWhiteSquare', '&#10215;');/*1129*/
    var LeftMiddleRight = P(MathCommand, function (_, super_) {
        _.init = function (ch, htmll, htmlm, htmlr) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<big>' + htmll + '</big>' +
                '<span class="mq-non-leaf ">&0</span>' +
    ...
    LatexCmds.Sandwich = bind(LeftMiddleRight, '\\Sandwich', '&#9001;', '&#124;', '&#9002;');
    ///////////////////////////////////////////////////////////////////////////
    //for 15
    var Upsignal = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                /* '<span class="mq-large-operator mq-non-leaf">' +*/
                /*added later*/
    ...
    /*later*/
    var Upsignal_1 = P(Upsignal, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                /* '<span class="mq-large-operator mq-non-leaf">' +*/
                /*added later*/
                // '<span class="mq-large-operator mq-non-leaf" style="vertical-align: 2em;">' +
                // '<span class="mq-large-operator mq-non-leaf" style="vertical-align: 1em;">' +
                '<span class="mq-large-operator mq-non-leaf" style="vertical-align: 0.8em;">' +
                //	'<span class="mq-to"><span>&1</span></span>' +
                '<span>' + '<big>' + html + '</big>' + '</span>' +
                '<span class="mq-from"><span>&0</span></span>' +
                '</span>';
    ...
    //15
    //02
    LatexCmds.intupdown = bind(SummationNotation, '\\intupdown', '&#8747;');
    //05
    LatexCmds.intup = bind(Upsignal, '\\intup', '&#8747;');
    //07
    LatexCmds.doubleint = bind(BinaryOperator, '\\doubleint', '&#8748;');
    //08
    LatexCmds.doubleintup = bind(Upsignal, '\\doubleintup', '&#8748;');
    //10 11
    LatexCmds.tripleint = bind(BinaryOperator, '\\tripleint', '&#8749;');
    LatexCmds.tripleintup = bind(Upsignal, '\\tripleintup', '&#8749;');
    //13 14
    LatexCmds.contourint = bind(BinaryOperator, '\\contourint', '&#8750;');
    LatexCmds.contourintup = bind(Upsignal, '\\contourintup', '&#8750;');
    //16 17
    LatexCmds.surfaceint = bind(BinaryOperator, '\\surfaceint', '&#8751;');
    LatexCmds.surfaceintup = bind(Upsignal, '\\surfaceintup', '&#8751;');
    //19 20
    LatexCmds.volumeint = bind(BinaryOperator, '\\volumeint', '&#8752;');
    LatexCmds.volumeintup = bind(Upsignal, '\\volumeintintup', '&#8752;');
    //22 23
    LatexCmds.AntiClockContourInt = bind(BinaryOperator, '\\AntiClockContourInt', '&#8755;');
    LatexCmds.AntiClockContourIntup = bind(Upsignal, '\\AntiClockContourIntup', '&#8755;');
    //25 26
    LatexCmds.ClockContourInt = bind(BinaryOperator, '\\ClockContourInt', '&#8754;');
    LatexCmds.ClockContourIntup = bind(Upsignal, '\\ClockContourIntup', '&#8754;');
    //////////////////////////////////////////////////
    //17
    // var Upsignal = P(MathCommand, function(_, super_) {
    // 	_.init = function(ch, html) {
    ...
    /*继承MathCommand类，并重写相应的方法*/
    var Downsignal = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<span class="mq-to"><span>&1</span></span>' +
    ...
    /*later*/
    var Downsignal_1 = P(Downsignal, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf" style="vertical-align: -1em;">' +
                '<span class="mq-to"><span>&1</span></span>' +
    ...
    var Downsignal_2 = P(Downsignal, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf" style="vertical-align: 1em;">' +
                '<span class="mq-to"><span>&1</span></span>' +
    ...
    /*later*/
    /*#17:01 - 09*/
    LatexCmds.DownRightArrow = bind(Downsignal, '\\DownRightArrow', '&#8594;');
    LatexCmds.UpRightArrow = bind(Upsignal_1, '\\UpRightArrow', '&#8594;');/*later*/
    LatexCmds.UpDownRightArrow = bind(SummationNotation_1, '\\UpDownRightArrow', '&#8594;');/*later*/
    LatexCmds.DownLeftArrow = bind(Downsignal, '\\DownLeftArrow', '&#8592;');
    LatexCmds.UpLeftArrow = bind(Upsignal_1, '\\UpLeftArrow', '&#8592;');/*later*/
    LatexCmds.UpDownLeftArrow = bind(SummationNotation_1, '\\UpDownLeftArrow', '&#8592;');/*later*/
    LatexCmds.DownLeftRightArrow = bind(Downsignal, '\\DownLeftRightArrow', '&#8596;');
    LatexCmds.UpLeftRightArrow = bind(Upsignal_1, '\\UpLeftRightArrow', '&#8596;');/*later*/
    LatexCmds.UpDownLeftRightArrow = bind(SummationNotation_1, '\\UpDownLeftRightArrow', '&#8596;');/*later*/
    /*
	var Doublesignal = P(MathCommand, function(_, super_) {
	  _.init = function(ch, htmlup, htmldown) {
	    var htmlTemplate =
	    '<span class="mq-large-operator mq-non-leaf">' +
	      '<span class="mq-to"><span>&1</span></span>' +
	      '<span class="mq-up">'+ htmlup +'</span>' +
	      '<span class="mq-down">'+ htmldown +'</span>' +
	      '<span class="mq-from"><span>&0</span></span>' +
	      '</span>'
	    Symbol.prototype.init.call(this, ch, htmlTemplate);
	  };
	...
	LatexCmds.DoubleArrow = bind(Doublesignal, '\\DoubleArrow ', '&#8594;', '&#8592;');
	*/
    /*17xx*/
    LatexCmds.DownDoubleArrow = bind(Downsignal, '\\DownDoubleArrow ', '&#8644;');
    LatexCmds.UpDoubleArrow = bind(Upsignal_1, '\\UpDoubleArrow ', '&#8644;');/*later*/
    LatexCmds.UpDownDoubleArrow = bind(SummationNotation_1, '\\UpDownDoubleArrow ', '&#8644;');/*later*/
    LatexCmds.seventeenthirteen = bind(Downsignal, '\\seventeenthirteen ', '\u2942');
    LatexCmds.seventeenfourteen = bind(Upsignal_1, '\\seventeenfourteen ', '\u2942');/*later*/
    LatexCmds.seventeenfifteen = bind(SummationNotation_1, '\\seventeenfifteen ', '\u2942');/*later*/
    LatexCmds.seventeensixteen = bind(Downsignal, '\\seventeensixteen ', '\u2944');
    LatexCmds.seventeenseventeen = bind(Upsignal_1, '\\seventeenseventeen ', '\u2944');/*later*/
    LatexCmds.seventeeneighteen = bind(SummationNotation_1, '\\seventeeneighteen ', '\u2944');/*later*/
    LatexCmds.seventeennineteen = bind(Downsignal, '\\seventeennineteen ', '\u21CB');
    LatexCmds.seventeentwenty = bind(Upsignal_1, '\\seventeentwenty ', '\u21CB');
    LatexCmds.seventeentwoone = bind(SummationNotation_1, '\\seventeentwoone ', '\u21CB');
    LatexCmds.seventeentwotwo = bind(Downsignal, '\\seventeentwotwo ', '\u218A');
    LatexCmds.seventeentwothree = bind(Upsignal_1, '\\seventeentwothree ', '\u218A');/*later*/
    LatexCmds.seventeentwofour = bind(SummationNotation_1, '\\seventeentwofour ', '\u218A');/*later*/
    LatexCmds.seventeentwofive = bind(Downsignal, '\\seventeentwofive ', '\u218B');
    LatexCmds.seventeentwosix = bind(Upsignal_1, '\\seventeentwosix ', '\u218B');/*later*/
    LatexCmds.seventeentwoseven = bind(SummationNotation_1, '\\seventeentwoseven ', '\u218B');/*later*/
    //11
    //last 4
    LatexCmds.TopCurly = bind(SummationNotation, '\\TopCurly', '&#9182;');
    LatexCmds.BottomCurly = bind(SummationNotation, '\\BottomCurly', '&#9183;');
    LatexCmds.TopSquare = bind(SummationNotation, '\\TopSquare', '&#9140;');
    LatexCmds.BottomSquare = bind(SummationNotation, '\\BottomSquare', '&#9141;');
    ////////////////////////////////////////////////////////////
    var Middle = P(MathCommand, function (_, super_) {
        _.init = function (ch, htmlm) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<span class="mq-non-leaf ">&0</span>' +
                '<big>' + htmlm + '</big>' +
    ...
    var MiddleSmall = P(MathCommand, function (_, super_) {
        _.init = function (ch, htmlm) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<span class="mq-scaled mq-sqrt-prefix">&0</span>' +
                '<big>' + htmlm + '</big>' +
                '<span class="mq-non-leaf ">&1</span>' +
                '</span>';
            Symbol.prototype.init.call(this, ch, htmlTemplate);
    ...
    /*var SqrtUpDown = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<span class="mq-to"><span>&1</span></span>' +
                ...
                '<span class="mq-non-leaf mq-sqrt-stem">&0</span>' + '</span>';
                // '<span class="mq-non-leaf mq-sqrt-stem">&0</span>' + '</span>';
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };
        _.createLeftOf = function (cursor) {
    ...
    var Longdiv = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<span class="mq-scaled mq-sqrt-prefix" style="vertical-align: 0.35em;">\u27cc</span>' + /*later*/
                '<span class="mq-non-leaf mq-sqrt-stem">&0</span></span>';
            Symbol.prototype.init.call(this, ch, htmlTemplate);
        };
        _.createLeftOf = function (cursor) {
    ...
    var LongdivTwo = P(MathCommand, function (_, super_) {
        _.init = function (ch, html) {
            var htmlTemplate =
                '<span class="mq-large-operator mq-non-leaf">' +
                '<span class="mq-to"><span>&1</span></span>' +
    ...
    // var SqrtUp = P(MathCommand, function(_, super_) {
    // 	_.init = function(ch, html) {
    // 		var htmlTemplate =
    // 			'<span class="mq-large-operator mq-non-leaf">' +
    // 			'<span class="mq-to"><span>&1</span></span>' +
    ...
    /* \u27cc:⟌   */
    /*  var Longdiv = LatexCmds.longdiv = LatexCmds['\u27cc'] = P(MathCommand, function(_, super_) {
    _.ctrlSeq = '\\longdiv';
    _.htmlTemplate = '<span class="mq-non-leaf">' +
        //'<span class="mq-scaled mq-sqrt-prefix">&radic;</span>' +
        '<span class="mq-non-leaf mq-sqrt-stem">&0</span>' +
        '</span>';
    _.textTemplate = ['longdiv(', ')'];
    _.parser = function() {
      return latexMathParser.optBlock.then(function(optBlock) {
        return latexMathParser.block.map(function(block) {
          var nthroot = NthRoot();
    ...
    // var Doublesignalsingle = P(MathCommand, function(_, super_) {
    //   _.init = function(ch, htmlup, htmldown) {   //初始化
    //     var htmlTemplate =
    //     '<span class="mq-large-operator mq-non-leaf">' +
    //      // '<span class="mq-to"><span>&1</span></span>' +
    //       '<span class="mq-up">'+ htmlup +'</span>' +
    ...
    /*12xx,14xx,16xx,18xx*/
    LatexCmds.summm = bind(Upsignal, '\\summm', '&sum;');
    LatexCmds.sumbase = bind(BinaryOperator, '\\sumbase', '&sum;');
    LatexCmds.fracc = bind(SummationNotation, '\\fracc', '&#45;');
    LatexCmds.solidus = bind(Middle, '\\solidus', '&#47;');
    LatexCmds.solidusss = bind(Middle, '\\solidusss', '&#10744;');
    LatexCmds.fraccc = bind(MiddleSmall, '\\fraccc', '&#10744;');
    LatexCmds.longdiv = bind(Longdiv, '\\Longdiv');
    LatexCmds.longdivtwo = bind(LongdivTwo, '\\Longdivtwo');
    LatexCmds.tildeup = bind(Upsignal, '\\tildeup', '&#126;');
    LatexCmds.uponarrow = bind(Upsignal, '\\uponarrow', '&#8963;');
    LatexCmds.toppar = bind(Upsignal, '\\toppar', '&#9180;');
    LatexCmds.toptor = bind(Upsignal, '\\toptor', '&#9184;');
    LatexCmds.overlineon = bind(Upsignal, '\\overlineon', '&#45;');
    LatexCmds.cmblineon = bind(Upsignal, '\\cmblineon', '&#61;');
    LatexCmds.overlinedown = bind(Downsignal_1, '\\overlinedown', '&#45;');/*later*/
    LatexCmds.cmblinedown = bind(Downsignal_1, '\\cmblinedown', '&#61;');/*later*/
    LatexCmds.rightarrowon = bind(Upsignal, '\\rightarrowon', '&#8594;');
    LatexCmds.leftarrowon = bind(Upsignal, '\\leftarrowon', '&#8592;');
    LatexCmds.leftrighton = bind(Upsignal, '\\leftrighton', '&#8596;');
    LatexCmds.rightarrowdown = bind(Downsignal_1, '\\rightarrowdown', '&#8594;');/*later*/
    LatexCmds.leftarrowdown = bind(Downsignal_1, '\\leftarrowdown', '&#8592;');/*later*/
    LatexCmds.leftrightdown = bind(Downsignal_1, '\\leftrightdown', '&#8596;');/*later*/
    LatexCmds.middleline = bind(Middle, '\\middleline', '&#8260;');
    LatexCmds.prodbase = bind(VanillaSymbol, '\\prodbase', '&prod;');
    LatexCmds.prodtwo = bind(Upsignal, '\\prodtwo', '&prod;');
    LatexCmds.coprodbase = bind(VanillaSymbol, '\\coprodbase', '&#8720;');
    LatexCmds.coprodtwo = bind(Upsignal, '\\coprodtwo', '&#8720;');
    LatexCmds.bigcuporigin = bind(Upsignal, '\\bigcuporigin', '&#8898;');
    LatexCmds.bigcupdouble = bind(SummationNotation, '\\bigcuporigin', '&#8898;');
    LatexCmds.unionorigin = bind(Upsignal, '\\unionorigin', '&#8899;');
    LatexCmds.uniondouble = bind(SummationNotation, '\\uniondouble', '&#8899;');
    /*13xx*/
    LatexCmds.thirteenseven = bind(Downsignal_2, '\\thirteenseven', '&0');
    LatexCmds.thirteeneight = bind(Upsignal_1, '\\thirteeneight', '&1');/*later*/
    /*LatexCmds.thirteennine = bind(SummationNotation1, '\\thirteennine','&0');*/
    /*20xx  added by Zhang Yibiao*/
    LatexCmds.twentyone = bind(Summ, '\\twentyone', '&#8968;');
    LatexCmds.twentytwo = bind(Summ1, '\\twentytwo', '&#8969;');
    LatexCmds.twentythree = bind(Summ, '\\twentythree', '&#8970;');
    LatexCmds.twentyfour = bind(Summ1, '\\twentyfour', '&#8971;');
    /*1xx*/
    LatexCmds.trianglepoint = bind(Downsignal, '\\trianglepoint', '&#9652;');
    LatexCmds.twozerotwo = bind(Downsignal, '\\twozerotwo', '&#8896;');
    LatexCmds.twozerothree = bind(Downsignal, '\\twozerothree', '&#9012;');
    LatexCmds.twoonezero = bind(VanillaSymbol, '\\twoonezero', '\u22F0');
    LatexCmds.oneonesix = bind(VanillaSymbol, '\\oneonesix', '\u2259');
    /*03xx*/
    LatexCmds.pprim = bind(VanillaSymbol, '\\pprim', '&#8243;');
    // LatexCmds.ppprim = bind(VanillaSymbol, '\\ppprim', '&#8244;');
    // LatexCmds.tprime = bind(VanillaSymbol, '\\tprime', '&tprime;');/*later*/
    LatexCmds.ppprim = bind(VanillaSymbol, '\\ppprim', '\u2034');/*later*/
    LatexCmds.xprim = bind(VanillaSymbol, '\\xprim', '&#8245;');

    LatexCmds.onepoint = bind(Upsignal_1, '\\onepoint', '&#8228;');/*later*/
    LatexCmds.twopoint = bind(Upsignal_1, '\\twopoint', '&#8229;');/*later*/

    // LatexCmds.threepoint = bind(Upsignal, '\\threepoint', '&#8411;');
    LatexCmds.threepoint = bind(Upsignal_1, '\\threepoint', '&hellip;');/*later*/
    LatexCmds.fourpoint = bind(Upsignal, '\\fourpoint', '&#8412;');/*later*/
    LatexCmds.pointone = bind(Downsignal, '\\pointone', '&#8228;');
    LatexCmds.pointtwo = bind(Downsignal, '\\pointtwo', '&#8229;');

    // LatexCmds.pointthree = bind(Downsignal, '\\pointthree', '&#8411;');
    LatexCmds.pointthree = bind(Downsignal, '\\pointthree', '&hellip;');/*later*/

    LatexCmds.pointfour = bind(Downsignal, '\\pointfour', '&#8412;');
    LatexCmds.threetwoone = bind(Upsignal_1, '\\threetwoone', '&#45;');/*later*/
    LatexCmds.threetwotwo = bind(Upsignal_1, '\\threetwotwo', '&#126;');/*later*/
    LatexCmds.threetwothree = bind(Upsignal_1, '\\threetwothree', '&#8994;');/*later*/
    LatexCmds.threetwofour = bind(Upsignal_1, '\\threetwofour', '&#8995;');/*later*/
    LatexCmds.threetwofive = bind(Upsignal_1, '\\threetwofive', '&and;');/*later*/
    LatexCmds.threetwoonedown = bind(Downsignal, '\\threetwoonedown', '&#45;');
    LatexCmds.threetwotwodown = bind(Downsignal, '\\threetwotwodown', '&#126;');
    LatexCmds.threetwothreedown = bind(Downsignal, '\\threetwothreedown', '&#8994;');
    LatexCmds.threetwofourdown = bind(Downsignal, '\\threetwofourdown', '&#8995;');
    LatexCmds.threethreeone = bind(Upsignal_1, '\\threethreeone', '&rarr;');/*later*/
    LatexCmds.threethreetwo = bind(Upsignal_1, '\\threethreetwo', '&larr;');/*later*/
    LatexCmds.threethreethree = bind(Upsignal_1, '\\threethreethree', '&#8596;');/*later*/
    LatexCmds.threethreefour = bind(Upsignal_1, '\\threethreefour', '&#8640;');/*later*/
    LatexCmds.threethreefive = bind(Upsignal_1, '\\threethreefive', '&#8636;');/*later*/
    LatexCmds.threethreesix = bind(Downsignal, '\\threethreesix', '&rarr;');
    LatexCmds.threethreeseven = bind(Downsignal, '\\threethreeseven', '&larr;');
    LatexCmds.threethreeeight = bind(Downsignal, '\\threethreeeight', '&#8596;');
    // LatexCmds.threethreenine = bind(Downsignal, '\\threethreenine', '&#8640;');
    LatexCmds.threethreenine = bind(Downsignal, '\\threethreenine', '&#8641;');
    LatexCmds.threefourzero = bind(Downsignal, '\\threefourzero', '&#8637;');
    /* 4xx*/
    LatexCmds.starpoint = bind(VanillaSymbol, '\\starpoint', '&#8270;');
    LatexCmds.leftanglebra = bind(VanillaSymbol, '\\leftanglebra', '&#10216;');
    LatexCmds.rightanglebra = bind(VanillaSymbol, '\\rightanglebra', '&#10217;');
    LatexCmds.leftwhitebra = bind(VanillaSymbol, '\\leftwhitebra', '&#10214;');
    LatexCmds.rightwhitebra = bind(VanillaSymbol, '\\rightwhitebra', '&#10215;');
    /*5xx*/
    LatexCmds.fivetwothree = bind(VanillaSymbol, '\\fivetwothree', '\u218A');
    LatexCmds.fivetwofour = bind(VanillaSymbol, '\\fivetwofour', '\u218B');
    LatexCmds.fivethirteen = bind(VanillaSymbol, '\\fivethirteen', '&#10530;');
    LatexCmds.fivesixteen = bind(VanillaSymbol, '\\fivesixteen', '&#10529;');
    LatexCmds.fivenineteen = bind(VanillaSymbol, '\\fivenineteen', '&#8644;');
    LatexCmds.fivetwenty = bind(VanillaSymbol, '\\fivetwenty', '&#10562;');
    LatexCmds.fivetwentyone = bind(VanillaSymbol, '\\fivetwentyone', '&#10564;');
    LatexCmds.fivetwotwo = bind(VanillaSymbol, '\\fivetwotwo', '&#8652;');
    // LatexCmds. = bind(,'\\','&#;');
    // LatexCmds. = bind(,'\\','&#;');
    LatexCmds.fivetwentysix = bind(VanillaSymbol, '\\fivetwentysix', '&#8629;');
    /*06xx*/
    LatexCmds.sixsix = bind(VanillaSymbol, '\\sixsix', '&#8989;');
    /*08xx*/
    LatexCmds.eightzerosix = bind(VanillaSymbol, '\\eightzerosix', '\u211D');
    LatexCmds.eightzeroseven = bind(VanillaSymbol, '\\eightzeroseven', '\u2124');
    LatexCmds.eightzeroeight = bind(VanillaSymbol, '\\eightzeroeight', '\u2102');
    LatexCmds.eightzeronine = bind(VanillaSymbol, '\\eightzeronine', '\u211A');
    LatexCmds.eightten = bind(VanillaSymbol, '\\eightten', '\u2115');
    LatexCmds.eighttwosix = bind(VanillaSymbol, '\\eighttwosix', '&#8736;');
    LatexCmds.angsph = bind(VanillaSymbol, '\\angsph', '&#8738;');
    LatexCmds.eightthirteen = bind(VanillaSymbol, '\\eightthirteen', '\u019B');
    LatexCmds.mho = bind(VanillaSymbol, '\\mho', '\u028A');
    LatexCmds.eighttwotwo = bind(VanillaSymbol, '\\eighttwotwo', '&prod;');
    LatexCmds.eightthreefour = bind(VanillaSymbol, '\\eightthreefour', '&#9649;');
