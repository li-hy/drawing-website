 ...
 */
function addF2canvas() {
	saveAsPicture(function myShow(imageHTML) {
		delimiter = new RegExp('\\\'')
		tmp = imageHTML.split(delimiter);
		let src = tmp[3];
		let fTxt = tmp[5];
		window.parent.addFormula(window.parent.shapeLayer, src, fTxt);
	});
}
...
function closeWindow() {
	window.parent.$('#right-click-formula-editor').hide();
}
...
function reEdit() {
	mathField.focus();
	mathField.keystroke('Ctrl-A');
	mathField.write(window.parent.fTxt);
}
